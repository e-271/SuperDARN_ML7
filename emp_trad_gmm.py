...
def gmm_dist(rad, beam, stm, etm):
	...
	plot_rti_emp(rad, beam, stm, etm, data_dict,'velocity',fig_num=1,title_str='Empirical Model Results')
	...
	plot_rti(rad, beam, stm, etm, data_dict,'velocity',gsct=False,fig_num=2,title_str='Traditional Model Results')
...
def read_from_updated_db(rad, beam, stm, etm, baseLocation="/home/xueling/data/sqlite3/"):
	...
	command = "SELECT * FROM {tb}\
				...
				AND beam = '{beam}' \
    			ORDER BY time".\
				format(tb="sd_table_"+rad, stm=stm, etm=etm, beam=beam)
...
def plot_rti(rad, beam, stm, etm, data_dict,plot_param,gsct=False,gsfg_min_vel=0,fig_num=2,title_str=' '):
    ...
    ax.set_title(title_str+' '+stm.strftime("%d %b %Y") + ' ' + rad.upper() + ' Beam '+ str(beam))
...
def plot_rti_emp(rad, beam, stm, etm, data_dict,plot_param,fig_num=1,title_str=' '):
    # Now let's plot all data.
    rmax = data_dict['nrang'][0] #number of range gate, usually = 75 or 110
    tcnt = len(data_dict['datetime'])
